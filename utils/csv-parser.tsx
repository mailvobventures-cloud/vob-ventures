// Removed top-level fs and path imports to avoid client-side build errors
...
export async function fetchPortfolioData(): Promise<PortfolioItem[]> {
  ...
  try {
    ...
    if (typeof window === "undefined") {
      ...
      const filePath = join(process.cwd(), 'public', 'data', 'portfolio-sample.csv')
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    headers.forEach((header, index) => {
      ...
      if (key && index < values.length) {
        // @ts-ignore - We know this is safe for now as we parse string values
